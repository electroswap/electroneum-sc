...
import (
	...
	"github.com/electroneum/electroneum-sc/crypto/kzg4844"
	"github.com/electroneum/electroneum-sc/eth"
	"github.com/electroneum/electroneum-sc/eth/catalyst"
	...
	"github.com/electroneum/electroneum-sc/trie/triedb/hashdb"
	"github.com/electroneum/electroneum-sc/trie/triedb/pathdb"
...
var (
	...
	NetworkIdFlag = &cli.Uint64Flag{
		Name:     "networkid",
		Usage:    "Explicitly set network id (integer)(For Electroneum stagenet/testnet: use --goerli, --sepolia instead)",
	...
	// There is deliberately no --holesky: Electroneum has three networks, and
	// upstream's flag had no genesis case here, so passing it silently produced
	// mainnet configuration under an Ethereum testnet's name and datadir.
	// Dev mode
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	// Electroneum's own networks reuse upstream's testnet flags rather than
	// introducing new ones, so --sepolia selects ETN testnet (5201420) and
	// --goerli selects stagenet (5201419).
	case ctx.Bool(SepoliaFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
